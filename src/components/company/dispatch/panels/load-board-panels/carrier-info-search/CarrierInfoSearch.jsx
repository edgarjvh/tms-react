...
function CarrierInfoSearch(props) {
    ...
    const rowDoubleClick = async (e, c) => {
        await props.setSelectedLbCarrierInfoCarrier(c);
        ...
        if (c.drivers.length > 0) {
            await props.setSelectedLbCarrierInfoDriver(c.drivers[0]);
        }

        closePanelBtnClick(null, 'lb-carrier-info-search');
    }

    return (
        <div className="panel-content">
            <div className="drag-handler" onClick={e => e.stopPropagation()}></div>
            <div className="close-btn" title="Close" onClick={e => closePanelBtnClick(e, 'lb-carrier-info-search')}><span className="fas fa-times"></span></div>
            <div className="title">{props.title}</div><div className="side-title"><div>{props.title}</div></div>

            <div className="input-box-container" style={{ marginTop: 20, display: 'flex', alignItems: 'center' }}>
                {
                    (props.lbCarrierInfoCarrierSearch || []).map((item, index) => {
            ...
            <div className="tbl">
                ...
                <div className="tbody">
                    <div className="tbody-wrapper">
                        {
                            props.lbCarrierInfoCarriers.length > 0
                                ? props.lbCarrierInfoCarriers.map((c, i) => {
                                    return (
